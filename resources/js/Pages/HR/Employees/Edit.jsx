...
export default function Edit({ auth, employee, clusters, departments }) {
  // Use Inertia's useForm with all editable fields
  ...
  const handleSubmit = (e) => {
    ...
    // Use Inertia's put method with FormData for file upload
    const formData = new FormData();
    Object.keys(data).forEach((key) => {
      if (data[key] !== null && data[key] !== undefined && data[key] !== '') {
        // For boolean/numbers, convert to string
        formData.append(key, data[key]);
      }
    });
    // Append _method to spoof PATCH
    formData.append('_method', 'PATCH');

    // Use Inertia's post with the FormData
    router.post(route('hr.employees.update', employee.id), formData, {
      ...
      onError: (errors) => {
        ...
        // Errors are automatically populated in the form
      },
    });
  };
          ...
          <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                disabled={processing}
                className="inline-flex items-center rounded-md bg-navy-700 px-4 py-2 text-sm font-medium text-white hover:bg-navy-800 disabled:opacity-50"
              >
          ...
          <form onSubmit={handleSubmit} encType="multipart/form-data">
                ...
                <div className="flex flex-col items-center gap-4 sm:flex-row">
                  <div className="relative flex-shrink-0">
                    <img
                      src={photoPreview}
                      alt="Profile photo"
                      className="h-32 w-32 rounded-full object-cover ring-4 ring-navy-100"
                      onError={(e) => { e.target.src = defaultAvatar; }}
                    />
                    <label
                      htmlFor="photo-upload"
                      className="absolute bottom-0 right-0 cursor-pointer rounded-full bg-navy-700 p-1.5 text-white hover:bg-navy-800"
                    >
                    ...
                    {data.profile_photo && (
                      <button
                        ...
                        onClick={() => {
                          ...
                          setPhotoPreview(employee.profile_photo_url || defaultAvatar);
                        }}
            ...
            <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-2">
              ...
              <div className="rounded-xl bg-white p-4 shadow-sm sm:p-6">
                ...
                <p className="text-sm text-gray-500">
                  Password management is done separately via the "Reset Password" action on the profile page.
                </p>
              </div>
            </div>
